                              ...
                              FbUpdateRect*   rect)
{
    ...
    //    printf("h=800, xx1=480, yy is =%d, width = %d, ymin = %d, ymax = %d \n", yy, width, rect->ymin, rect->ymax);
    NEXT_LINE:
    ...
    //    printf("if rect ymin is greater than rect ymax\n");
        return -1;
    }
...
static void goldfish_fb_update_display(void *opaque)
{
    ...
    * the host systems memory. Now, next iteration we check if the application has changed the data in the src_line pointed memory region.
    ...
    *///GemDroid end "comments"
	...
	//GemDroid added
    //FB place 2 : change detected in display state. Copy to DRAM memory mapped Framebuffer location
    if(IP_tracer)
	{
...
//        printf("ymin= %d, ymax= %d\n", rect.ymin, rect.ymax);
        ...
        else if (compute_fb_update_rect_linear(&fbs, base, &rect) == 0)
        {
  //      printf("ymin= %d, ymax= %d\n", rect.ymin, rect.ymax);
